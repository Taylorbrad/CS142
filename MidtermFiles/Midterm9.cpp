...
int main()
{
    ...
    string garbageComma = "";
    /* FIX CONSTANTS */
    const string monthArray[] = {"January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"};
    const string weekDaysArray[] = {"Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"};
    ...
    while (!isInputValid)
    {
        ...
        cout << weekDaysArray[getWeekDay(daysBetweenDates)];
...
int getTotalDaysUntilInput(int yearNum, int monthNum, int dayNum, int* monthDaysArray)
{
    ...
    for (;yearNum > 1583 /*magic numbrero*/; --yearNum)
    {
        if (determineIfLeapYear(yearNum))
        {
            totalDays += 366;
        }
        else
    ...
    for (;monthNum > 1 /*magic numbrero*/; --monthNum)
    {
        totalDays += monthDaysArray[monthNum - 1];
    }
    for (;dayNum > 1; --dayNum)
    {
        totalDays += 1;
