...
int main()
{
	...
	while (monthNum != 0)
	{
		...
		if (isInputValid)
		{
		    ...
		    cout << weekDaysArray[getWeekDay(daysBetweenDates)] << endl << endl;
		}
	}
	system("pause");
	return 0;
...
int getTotalDaysUntilInput(int yearNum, int monthNum, int dayNum, int* monthDaysArray)
{
	...
	{
		totalDays += monthDaysArray[monthNumTemp - 2];
		//cout << monthNumTemp;
		...
		//cout << "Added " << monthDaysArray[monthNumTemp - 2] << "Days according to month #" << monthNumTemp - 1;
	}
